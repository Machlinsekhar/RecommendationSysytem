...
def extract_data_for_restaurant(file_path, user_location):
    config = load_config()
    rest_names = []
    torestaurants = []
    toreviews = []
    ...
    with open(file_path, 'r') as file:
        ...
        try:
            with psycopg2.connect(**config) as conn:
                ...
                cur = conn.cursor()
                check_sql = f"""
                SELECT EXISTS(SELECT 1 FROM test.locations WHERE loc_name='{user_location}')
                """
                ...
                if bool_val:
                    ...
                    for restaurant in data:
                        ...
                        VALUES('{torestaurants[0][0]}','{torestaurants[0][1]}',1);
                        """
...
def main_connect(location):
    file_path = 'places-of-top-restaurants-in-pune.json'
